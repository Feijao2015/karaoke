              ...
              {ranking.map((item, index) => (
                <tr key={index}>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="w-8 h-8 rounded-full bg-karaoke-primary text-white flex items-center justify-center">
                      {index + 1}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.singer}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.song.title} - {item.song.artist}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDate(item.date)}</td>
                  <td className={`px-6 py-4 whitespace-nowrap text-sm font-semibold text-right ${getScoreColor(item.score)}`}>
                    {item.score}
                  </td>
                </tr>
              ))}
